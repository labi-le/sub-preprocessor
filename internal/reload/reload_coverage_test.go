...
var reloadClassification = map[string]string{
	...
	// The odd pair in this block, and deliberately so: every geo.* sibling
	// above is liveProcessor because OptionsFromConfig carries it into
	// preprocess.NewProcessor, but geo.cloudflare's only consumer is the stable
	// worker's post-probe stage (filterAndMeasureEgress -> TraceCheck). The
	// class follows the CONSUMER, not the namespace; a reader of geo.* cannot
	// infer it, and inheriting the siblings' class would stop a timeout edit
	// reaching the RUNNING worker. config.ProberChanged is the gate.
	"geo.cloudflare.timeout":     liveWorker,
	"geo.cloudflare.concurrency": liveWorker,
	...
	"geoblock.gemini.rate_limit":   liveWorker,
...
var keyMutators = map[string]func(*config.Config){
	...
	"geo.cloudflare.timeout":        func(c *config.Config) { c.Geo.Cloudflare.Timeout = 30 * time.Second },
	"geo.cloudflare.concurrency":    func(c *config.Config) { c.Geo.Cloudflare.Concurrency = 4 },
	...
	"filters[].rate_limit":  func(c *config.Config) { c.Filters[3].RateLimit = 60 },
	...
	// an unloadable value it pins three gates rather than one. config.Equal
	// must reach AnnotateSpec.Tag (or mutatedReach rejects the mutator as
	// proving nothing), OptionsFromConfig must carry the field into the
	// processor's Options (proc), and config.AnnotateChanged must see it so the
	// worker re-applies (worker) — the liveBoth class fails on either half.
	...
	"geoblock.gemini.rate_limit":   func(c *config.Config) { c.GeoBlock.Gemini.RateLimit = 60 },
...
var reloadGateFixtures = []struct {
	...
	// The trap this fixture exists for: geo.cloudflare sits among geo.* keys
	// that are processor-side, and only the worker consumes it. Written as a
	// real YAML edit under `geo:`, so it proves the running worker is
	// re-configured (ctl.Apply ran) rather than merely that a processor was
	// rebuilt -- which the "unrelated" resolver.timeout row below shows is a
