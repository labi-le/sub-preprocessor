...
func TestMetricsObserveRender(t *testing.T) {
	...
	// The gemini gate no longer renders a family of its own, and nothing
	// about the nodes it could not verify may appear as a drop reason: those
	// nodes were KEPT and published.
	if strings.Contains(out, "stable_gemini_gate_") {
		t.Errorf("the gemini gate's private series are gone:\n%s", out)
	}
...
// TestMetricsPrecheckStatesRenderApart:  a breaker that discarded the verdict condemns nobody, so
