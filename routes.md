...
**Key functions:**
- `NodeName(b, node, tags string)` — write the node into a reusable `bytes.Buffer` with the already-formatted tag prefix (e.g. `"[GEO:NL][ASN:AS64500 EXAMPLE]"`) folded into its name; empty `tags` reduces to a clean relabel; vmess folds into the base64 `ps` and ssr into the base64 `remarks` query value (`subscription.RewriteVmessName` / `RewriteSSRName`), every other scheme into the `#fragment`. For ssr the fragment path is not merely unused but corrupting — mihomo base64-decodes the appended `#name` along with the payload and the link becomes "format invalid" — and a payload neither rewriter can decode is written verbatim: unannotated beats mangled. The tag string is assembled by the preprocess annotator.
