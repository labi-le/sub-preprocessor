...
- `CheckerSpec` — the config-derived half of a `Checker` (`Sources`, `Allowed`, `Interval`, `Rounds`, `MaxFail`, `MaxAvgMs`, `SourceTimeout`, `Prober`, `Filters`), held in an `atomic.Pointer` and read once per cycle so a reload configures the NEXT cycle instead of rewriting the running one
