...
`./internal/subscription/subscription.go`, `xray.go`, `vmess.go`, `ss.go`, `ssr.go`, `mieru.go`
...
- `ProbeResult` / `Survivor` — per-node probe aggregate and selected node with mean delay; `Survivor.Mbps` holds the bandwidth filter's measured speed (0 when the filter is off) and `Survivor.Egress` (`preprocess.Egress`) what the node reported about itself through `TraceCheck` (zero when no trace ran or none answered)
...
- `Prober` — `Probe(ctx, payload) (map[string]ProbeResult, error)`; implemented by `MihomoProber`. The map is keyed by **entry label**, not by mihomo proxy name, and holds only nodes that succeeded at least once. A `mierus://` entry arrives as several proxies (one per configured port) that all fold onto one label, and `foldProbeResults` keeps the BEST of them (`betterProbe`: more successful rounds, lower mean as tiebreak) — best-of, never a sum, since mieru dials one of its ports, and adding N ports' rounds together would let `Successes` exceed `check.rounds` and walk straight through `SelectSurvivors`' `maxFail` gate. Iterating the proxies rather than the accumulator map is what makes a tie resolve in payload order instead of by map-iteration chance
...
- `CycleReport` / `SourceReport` / `FilterReport` / `TraceReport` — per-cycle accounting (aggregate counts + per-source preprocess drops + per-filter in/kept/dropped-by-reason + kept-node speeds + duration + the trace stage), assembled by `RunOnce` from data otherwise only logged. `SourceReport` mirrors `preprocess.Stats` field for field (`DNSDrop`/`GeoDrop`/`ASNDrop`/`GeoBlockDrop`/`IPv6Drop`/`Unsupported`). `TraceReport` is `Answered`/`Unanswered`/`Moved`, where `Moved` counts published countries that differ from what the offline chain gives for the RESOLVED address — the number the trace exists to justify, bounded in trustworthiness by the answered share
...
**Key functions:**
- `Merge(bodies []SourceBody) []Entry` — parse each kept node line on its own, dedupe by lowercased `server:port` first-wins in source order (`Entry.Addr` shares the lowercased key), relabel to `<source>-NNN`, and carry `NodeResult.IP` onto the entry. No annotation and no country decision happen here
...
**Key types:**
- `Result{Nodes int, Expired bool}` — `(Result).Live()` reports `Nodes > 0 && !Expired`; `(Result).Reason()` names WHY as an enumerable `Reason` (`live` / `expired` / `nodeless-2xx`), derived from the same two fields rather than stored, so no `Result` can carry a verdict contradicting its counts. `Expired` outranks a zero node count: an origin-advertised expiry is a death verdict, a nodeless 2xx is not
