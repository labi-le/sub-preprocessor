...
- `PipelineContext` — request-scoped state shared across filters (`Lookup`, `Allowed`, `Denied`, `Resolved`, `Scratch`, `Stats`, plus the unexported `sink`); `Scratch` is the per-node IP slice handed to filters (they compact in place), keeping the `Resolved` cache pristine across nodes sharing a server. `Lookup` is `Processor.countryChain`: the in-memory country databases the configured `GEO` annotate chain names, walked in that order, first answer wins
