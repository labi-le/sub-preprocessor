...
- `parseOriginRecord(txt) (uint32, geofeed.CountryCode, error)` — one `origin.asn.cymru.com` TXT record. **Field 0 is a space-separated AS LIST, not one number**: a multi-origin prefix reads `"15169 43515 | 35.212.128.0/17 | US | arin | …"`, and parsing it whole used to fail the record and discard the country in field 2 — costing the annotate chain a hit and leaving `ASNFilter` fail-open, since a `Resolve` error keeps the IP. The first listed AS is taken, because the caller needs exactly one number for the `AS<n>.asn.cymru.com` name lookup and the record ranks nothing; the country is a property of the single registration and survives a list of any length. **The AS NAME does not**, and that is the residual cost: the name feeds the `{type: asn}` deny patterns, so on a prefix whose ASes belong to different operators a pattern written for any but the first silently misses. Measured on live records: **28 of 50** sampled multi-origin prefixes list ASes of unrelated operators (`188.72.253.0/24` = AS7979 Servers.com + AS35415 Webzilla B.V.), the resolver's own Yandex fixture among them (`87.250.224.0/19` = AS13238 YANDEX + AS208398 Edge Technology Plus, RS). It is still strictly better than the state it replaced, which missed every AS on such a prefix AND the country
