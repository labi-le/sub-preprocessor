...
- `AnnotateSpec` — one entry in the ordered `annotate:` list: `tag` (`GEO`/`ASN`) + `providers` (ordered lookup chain over `geotrace|geofeed|dbip|registry|asn`, first provider that answers wins; required for both — defaulted to `[geofeed]` / `[asn]`; unknown and duplicate providers rejected). Any other tag is rejected as unknown, `IP` included: that tag was removed outright (nothing consumed it, no config selected it), though `rewrite.isKnownTag` still STRIPS an upstream-authored `[IP:…]`. `geotrace` is the one non-offline provider: it answers what the node reported about its own egress, which only the worker's post-probe stage has, so on `GET /` it always misses and the chain falls through. The retired singular `provider` key needs no field: the strict decode rejects it by name, as it does every future rename
...
Node output rewriting. Prepends the annotator's tag prefix (`[GEO:XX][ASN:…]`) before the node name. Strips existing known tags — deliberately a WIDER set than it writes, see `StripKnownTags`.

**Key functions:**
- `NodeName(b, node, tags string)` — write the node into a reusable `bytes.Buffer` with the already-formatted tag prefix (e.g. `"[GEO:AE][ASN:VDSINA - SERVERS TECH FZCO, AE]"`) folded into its name; empty `tags` reduces to a clean relabel; vmess folds into the base64 `ps` and ssr into the base64 `remarks` query value (`subscription.RewriteVmessName` / `RewriteSSRName`), every other scheme into the `#fragment`. For ssr the fragment path is not merely unused but corrupting — mihomo base64-decodes the appended `#name` along with the payload and the link becomes "format invalid" — and a payload neither rewriter can decode is written verbatim: unannotated beats mangled. The tag string is assembled by the preprocess annotator. An `[ASN:…]` value is a Cymru AS **name** with no `AS<number>` prefix — `annotTag.lookupASN` renders `geo.Info.ASN`, which `geo.asnProvider` copies from `asn.Result.Name`, i.e. `asn.parseASRecord`'s trailing record field — so a `[ASN:AS64500 …]` shape exists only in test stubs, never in output.
- `StripKnownTags(s) string` — remove the leading `[GEO:…]`, `[IP:…]`, `[ASN:…]`, `[SPD:…]`, `[OK]`, `[BAD]`, `[JUR:…]` tags (`isKnownTag`), skipping the whitespace between them. Only THREE of those are ever written by this service — `[GEO:]`/`[ASN:]` from the configured annotate tags (`config.TagGEO`/`TagASN`) and `[SPD:]` from `stable.speedPrefix`; `[IP:…]`, `[OK]`, `[BAD]` and `[JUR:…]` are recognised so that an upstream-authored tag is stripped on relabel instead of accumulating in front of ours. `[IP:…]` is the arm that outlived its writer — the `IP` annotate tag was removed — and it has to stay: the scan consumes a CONTIGUOUS run and returns the remainder from the first tag it does not recognise, so without it an upstream `[GEO:RU][IP:1.2.3.4] Moscow` loses only its GEO tag and gets republished as `[GEO:xx] [IP:1.2.3.4] Moscow`, carrying an address this service never verified
...
- `annotator` (`annotator.go`) — implements `Annotator`: builds the ordered `[GEO][ASN]` tag prefix for `req.IP`, then writes the relabeled node via `rewrite.NodeName`; nil when no tags are configured (annotation disabled, raw node emitted). GEO/ASN tags carry an ordered chain (`annotTag.chain` of `annotStep`, resolved against the map of providers the processor actually built — a referenced-but-missing name is logged and skipped, not panicked): the first step that answers wins; all-miss renders `[GEO:??]` / `[ASN:??]`. A step with a **nil** `prov` is `geotrace`: it is not a lookup, it repeats `req.Egress.Country` when `req.Egress.Valid()` and misses otherwise, so it is deliberately absent from the provider map (a name missing there is a wiring bug worth an Error log, and geotrace would trip it on every processor build, `/` included). It is always a miss for `ASN` — `cdn-cgi/trace` reports an address and a location, never an AS. `req.IP` is what every chain resolves against, but no tag renders the address itself any more: the dotted-quad `writeIP` writer went with the `IP` tag
