...
- `AnnotateSpec` — one entry in the ordered `annotate:` list: `tag` (`GEO`, the only accepted value) + `providers` (ordered lookup chain over `geotrace|geofeed|dbip|registry|asn`, first provider that answers wins; required, defaulted to `[geofeed]`; unknown and duplicate providers rejected). Any other tag is rejected as unknown, `IP` and `ASN` included: both tags were removed outright (nothing consumed them, no config selected them), though `rewrite.isKnownTag` still STRIPS an upstream-authored `[IP:…]` / `[ASN:…]`. One accepted tag does not collapse the list: entries render in order and may repeat, and an empty list disables annotation. Repetition is honoured by the RENDERING stage ONLY — `preprocess.countryChainOrder` reads the FIRST `GEO` entry's chain and returns, so a second entry publishes a tag the country FILTER never consulted (measured: `[geofeed]` then `[dbip]` renders `[GEO:??][GEO:DE]` and geo-drops the node under `countries=DE`, where one entry chaining `[geofeed, dbip]` keeps it and publishes `[GEO:DE]`; pinned by `TestCountryChainOrderDerivation`'s "a later GEO entry does not override the first"). `geotrace` is the one non-offline provider: it answers what the node reported about its own egress, which only the worker's post-probe stage has, so on `GET /` it always misses and the chain falls through. The retired singular `provider` key needs no field: the strict decode rejects it by name, as it does every future rename
...
- `(*Processor).countryChain(ctx) geofeed.CountryLookup` — the lookup the country filter judges with: the local databases the FIRST `GEO` annotate entry's chain names, in the order it names them (`chainLookup`, first non-zero answer wins; `countryChainOrder` derives the order once in `NewProcessor` and RETURNS on that first entry, so a repeated `GEO` entry feeds rendering only and can publish a country the filter never consulted). The filter used to see the geofeed alone while the GEO tag resolved through the whole annotate chain, so a node dbip places in DE was geo-dropped as unplaceable while its tag said `[GEO:DE]`; hardcoding geofeed → dbip → registry then reintroduced the same divergence for any operator who ordered the chain differently. The `asn` provider stays out — it is a per-IP Cymru round trip, and `{type: country, provider: asn}` already exposes it explicitly. `geotrace` stays out for a harder reason: it is not a lookup at all, it answers with what a node reported through ITS OWN proxy, and the IP stage runs before any proxy exists — so `{type: country}` remains offline and a `GEO` chain led by `geotrace` still filters on the local databases behind it. A geofeed-only chain, an asn-only chain and a config with no `GEO` entry all collapse to the geofeed lookup alone
