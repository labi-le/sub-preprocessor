...
- `AnnotateSpec` — one entry in the ordered `annotate:` list: `tag` (`GEO`, the only accepted value) + `providers` (ordered lookup chain over `cloudflare|geofeed|dbip|registry|asn`, first provider that answers wins; required, defaulted to `[geofeed]`; unknown and duplicate providers rejected — the retired `geotrace` spelling is rejected by that same check, with no back-compat). Any other tag is rejected as unknown, `IP` and `ASN` included: both tags were removed outright (nothing consumed them, no config selected them), though `rewrite.isKnownTag` still STRIPS an upstream-authored `[IP:…]` / `[ASN:…]`. One accepted tag does not collapse the list: entries render in order and may repeat, and an empty list disables annotation. Repetition reaches BOTH stages: rendering emits one tag per entry, and `preprocess.countryChainOrder` concatenates EVERY `GEO` entry's chain
