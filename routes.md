...
- `StripKnownTags(s) string` — remove the leading `[GEO:…]`, `[IP:…]`, `[ASN:…]`, `[SPD:…]`, `[OK]`, `[BAD]`, `[JUR:…]` tags (`isKnownTag`), skipping the whitespace between them. Only four of those are ever written by this service — `[GEO:]`/`[IP:]`/`[ASN:]` from the configured annotate tags (`config.TagGEO`/`TagIP`/`TagASN`) and `[SPD:]` from `stable.speedPrefix`; `[OK]`, `[BAD]` and `[JUR:…]` are recognised so that an upstream-authored tag is stripped on relabel instead of accumulating in front of ours
...
- `SourceBody` / `Entry` — merge input (source name + the `[]preprocess.NodeResult` its IP stage kept) and output. `Entry.Label` is the unique `<source>-NNN` string, and the key EVERY proxy↔entry map in this package uses (see `entryLabel`); `Entry.Raw` is the node line carrying that clean name — the one that is probed AND the one the publication annotates, since nothing is tagged before the cycle ends; `Addr` is the lowercased server:port dead-cache key; `IP` is the address the IP-filters judged, carried forward so the `[IP:]` tag and the offline GEO chain describe the same address the filter did (the worker never resolves a hostname twice); `Country` is filled ONLY at publication, from what the GEO chain returned — `"??"` when nothing resolved it, `""` when annotation is off or the line did not parse.
