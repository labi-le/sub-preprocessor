...
- `Checker` — the periodic worker loop. Besides the injected `Blocklist`/`DeadCache` it owns a third, private TTL set: the **reject cache** (`*DeadSet`, `rejectCacheTTL` = 6h, keyed `<filter>\x00<Entry.Addr>`). `NodeFilter.apply` returns the addresses it dropped on a verdict solid enough to reuse — in practice the store-backed API checks' refusals only; `tidal` (bare status code) and `bandwidth` (one shared-uplink dip puts a whole batch under the floor) deliberately return none — and `filterDead` skips a merged node when a filter that is still in the CURRENT chain AND currently `active()` rejected it recently. That is the only bookkeeping that can catch a through-node reject: `recordDead` marks only nodes ABSENT from the probe results, and a filter reject passed the probe by construction. Four things un-suppress a node: the TTL, dropping the filter from `filters` (the key is filter-scoped), the filter going inactive (a disabled filter drops nobody, so it may not suppress either), and any change to a filter's parameters — `Reconfigure` compares `CheckerSpec.FilterParams` and `Reset`s the whole cache, since a name cannot tell one `min_mbps` or endpoint from another. That reset is conditional on purpose: the crawler rewrites `private.yaml` hourly and the cycle interval is 1h, so resetting on every reload would leave the cache empty at every cycle, which is the churn it exists to stop.
- `CheckerSpec` — the config-derived half of a `Checker` (`Sources`, `Denied`, `Interval`, `Rounds`, `MaxFail`, `MaxAvgMs`, `SourceTimeout`, `Prober`, `Filters`, `FilterParams`), held in an `atomic.Pointer` and read once per cycle so a reload configures the NEXT cycle instead of rewriting the running one. `Denied` is the config-level country deny-set; the worker never imposes an allow-list, so it sends `filter.All()` as `AllowedCountries`. `FilterParams` (`NodeFilterParams`: the filter names in order + the merged gemini/claude/chatgpt/tidal/bandwidth params) is what `Reconfigure` diffs to decide whether the reject cache is still valid — a `NodeFilter` is an opaque closure and cannot be compared, and the geoblock `db_path`/`ttl` are deliberately left out because they configure the store, not a verdict
