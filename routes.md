...
- `GeoTraceConfig` — keyless params for the `geotrace` filter (`endpoint`/`timeout`/`concurrency`); defaults `https://cloudflare.com/cdn-cgi/trace`, 15s, 8. Not a gate: the filter it configures drops nothing and only corrects the `[GEO:]`/`[IP:]` tags with the egress the node reports about itself, so a bad endpoint costs accuracy, never survivors
...
- `(*Config).IPFilterSpecs() []IPFilterSpec` / `(*Config).NodeFilterSpecs() []NodeFilterSpec` — split the unified `filters` list into the IP-stage (country/asn) and through-node (gemini/claude/chatgpt/tidal/geotrace/bandwidth) specs the two builders consume
...
`./internal/stable/stable.go`, `merge.go`, `select.go`, `report.go`, `prober.go`, `prober_api.go`, `prober_gemini.go`, `prober_claude.go`, `prober_chatgpt.go`, `prober_tidal.go`, `prober_trace.go`, `prober_bandwidth.go`, `nodefilter.go`, `nodefilter_trace.go`, `label.go`, `checker.go`, `controller.go`, `progress.go`, `deadset.go`

Background worker that produces a stability-tested subscription list. Every `subscriptions.interval` it fetches each configured source through the geo/ASN pipeline (`Filterer`), merges the results into one deduplicated relabeled URI list, probes every node with the embedded mihomo library (`URLTest` HEAD requests, `check.rounds` rounds), keeps only nodes within `check.max_fail`/`check.max_avg_ms`, then runs the configured **through-node filters** on each surviving node (a real API `GET` whose body is classified — the check mihomo's HEAD-only `URLTest` cannot do): all but one are **gates** (Gemini/Claude/ChatGPT reachability, Tidal availability, bandwidth), each dropping what it refuses, while `geotrace` is deliberately not a gate and only retags the survivors with the egress they report. It records geo-blocked node hosts in the `geoblock` store (TTL) and drops them, and atomically publishes the rest for `GET /stable.txt`. Every failure mode (all sources down, zero parsable nodes, prober error, zero survivors) keeps the previous snapshot.
...
- `NodeFilter` — through-node check applied after the IP-filters + latency probe, routing THROUGH each surviving node (worker-only, so it shapes `/stable.txt`, not `/`); selected from the unified `filters` list via `cfg.NodeFilterSpecs()` (types `gemini`/`claude`/`chatgpt`/`tidal`/`geotrace`/`bandwidth`, of which only `geotrace` never drops). The interface is one method: `apply(ctx, survivors, proxies map[string][]mihomo.Proxy) (kept, report)` — proxies GROUPED by entry label, because one label can hold several proxies (mihomo expands a `mierus://` link into one per configured port) and `filterSubset` puts ALL of them into the check subset, so a port dead on our egress cannot mask a live sibling; the checks fold the OUTCOMES back onto the label instead (`betterAPIOutcome` / `betterBandwidthOutcome`, best-of-ports, resolved under the map's own mutex at write time so the result does not depend on which worker finished last). A drop lasts exactly this cycle — the checker remembers nothing a filter decides; a verdict worth outliving the cycle goes to the geoblock store, which the filter writes itself and which `preprocess` then honours before the node is ever merged. `buildNodeFilters(names, prober, store, annotate, logger)` constructs them; one generic `apiFilter{filterName, enabled, check, store}` implements the interface for gemini (key-gated) and claude/chatgpt/tidal (keyless), each keeping API-reachable survivors. A nil `enabled` func means always active; a disabled filter logs and keeps every survivor. gemini/claude/chatgpt record blocked hosts in the geoblock store; **tidal is built with a nil store on purpose** — its verdict is a bare status code, weaker than the AI checks' explicit refusal markers, and the store is host-keyed for its whole TTL, so one CDN hiccup would evict the node from every endpoint. With no store nothing outlives the cycle, so a rate-limited batch costs tidal's drop exactly one cycle.
- `TraceResult{Country, IP}` / `(*MihomoProber).TraceCheck(ctx, proxies) map[string]TraceResult` (`prober_trace.go`) — asks each node where its traffic actually leaves from, via Cloudflare's `/cdn-cgi/trace` (`key=value` lines; the body size is no property of the endpoint — it echoes the request User-Agent back in `uag=`, measured at 202 and 301 bytes for two of them — and nothing here sizes a buffer on it). `IP` is a fact — the address the endpoint saw the request arrive from; `Country` is Cloudflare's geo-IP lookup OF that address, still a database opinion but formed about the RIGHT address, which is the whole difference from the offline chain. Runs its own fan-out rather than `apiCheck`'s, because that one folds every response into a blocked/reachable verdict and discards the body, and four production gates depend on its signature. `parseTrace` accepts a result only when BOTH fields are present — a half-answer means the response did not come from the trace endpoint. A label covering several proxies (mieru's ports) folds by lowest proxy address, not by whichever goroutine finished first
- `geotraceFilter` (`nodefilter_trace.go`, type `geotrace`) — the only `NodeFilter` that is not a gate: it drops nothing and rewrites the `[GEO:]`/`[IP:]` values with what the node reported. The offline annotate chain places the address the RESOLVER returned, and 41% of the pool's named hosts sit in Cloudflare's shared anycast ranges (104.21/16, 172.67/16) which terminate in many countries at once — so the tag described Cloudflare's registration while the traffic left from the origin (a node tagged CA exiting in Germany). Measured on 102 live nodes: 47 tags corrected, 16 unanswered, and among named hosts the egress address differed from the resolved one in 12 of 18 cases. It only SUBSTITUTES tags already present (`swapTagValues` keeps every other tag and the order), so `annotate: []` disables it and it never introduces annotation the operator turned off; a node whose trace fails keeps the offline guess. `retagTraced` goes through `relabelNode`, reaching the name where each scheme keeps it — the vmess `ps` field, the ssr `remarks` query value, the URI fragment. It reports through `FilterReport.Notes`, never `Dropped` — `corrected` counts the nodes whose COUNTRY moved (decided from the parsed tag, not a substring of the line, which for vmess/ssr lives inside the base64 payload and would match nothing) and `unanswered` the ones the trace never reached; `Dropped` stays empty and `In == Kept`, so the drops chart never shows a filter that drops nothing. It also writes the corrected code back to `Entry.Country` (cloned — the trace body it is sliced from is up to `maxAPIBody`), or the kept-country gauge would keep reporting the anycast guess the published tag no longer carries
- The `bandwidth` `NodeFilter` (`bandwidthFilter`) downloads the bandwidth filter's `test_url` through each survivor (`BandwidthCheck` → `bandwidthProbeOne` → `measure`, `Accept-Encoding: identity` + body-transfer timing; `computeMbps` guards divide-by-zero), drops nodes below `min_mbps` and unreachable ones, records `Survivor.Mbps`, and — when annotation is enabled (`len(cfg.Annotate) > 0`) — prepends `[SPD:<n>M]` to the published name via `annotateSpeed` → `relabelNode`, which reaches the name where each scheme keeps it (the vmess `ps` field, the ssr `remarks` query value, or the URI `#fragment`) — an ssr line arriving here is already a `RewriteSSRName` product and appending `#<name>` to one destroys it rather than renaming it. `measure` reports a sample ONLY for a 2xx response read to completion, or one our own deadline cut short (a legitimate sample of a slow node): a non-2xx, an unfollowed redirect, or a body the peer truncated is a few fast kilobytes that `computeMbps` would inflate into a large fabricated rate, so it is reported not-reachable instead. It persists nothing: the concurrent downloads share one host uplink, so a dip on our side puts the whole batch under the floor, and next cycle re-measures.
