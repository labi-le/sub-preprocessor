...
- `GeoConfig` — `geo.geofeed` (`GeofeedConfig`) + `geo.dbip` (`DBIPConfig`) + `geo.registry` (`RegistryConfig`) + `geo.asn` (`ASNConfig`); the shared geo providers used by the country/asn filters and by annotation. Provider name constants: `ProviderGeofeed`/`ProviderDBIP`/`ProviderRegistry`/`ProviderASN`, plus `ProviderGeoTrace` — the one annotate provider with no `geo.*` block, because it is not a database: `geoblock.geotrace` configures its probe.
...
- `AnnotateSpec` — one entry in the ordered `annotate:` list: `tag` (`GEO`, the only accepted value) + `providers` (ordered lookup chain over `geotrace|geofeed|dbip|registry|asn`, first provider that answers wins; required, defaulted to `[geofeed]`; unknown and duplicate providers rejected). Any other tag is rejected as unknown, `IP` and `ASN` included: both tags were removed outright (nothing consumed them, no config selected them), though `rewrite.isKnownTag` still STRIPS an upstream-authored `[IP:…]` / `[ASN:…]`. One accepted tag does not collapse the list: entries render in order and may repeat, and an empty list disables annotation. Repetition reaches BOTH stages: rendering emits one tag per entry, and `preprocess.countryChainOrder` concatenates EVERY `GEO` entry's chain in written order, de-duplicated by first occurrence, so the country FILTER's provider set is the union of what the entries name. Splitting one chain over two entries therefore changes only the rendering — `[geofeed]` then `[dbip]` renders `[GEO:??][GEO:DE]` where one entry chaining `[geofeed, dbip]` renders `[GEO:DE]`, and both reach the same verdict. It used to change the verdict in both directions (`countries=DE` dropped the split node, `exclude_countries=DE` KEPT it); pinned by `TestEveryGEOEntryFeedsTheCountryFilter` and `TestCountryChainOrderDerivation`. `geotrace` is the one non-offline provider: it answers what the node reported about its own egress, which only the worker's post-probe stage has, so on `GET /` it always misses and the chain falls through. The retired singular `provider` key needs no field: the strict decode rejects it by name, as it does every future rename
...
- `GeoTraceConfig` — keyless params for the egress probe behind the `geotrace` ANNOTATE provider (`endpoint`/`timeout`/`concurrency`); defaults `https://cloudflare.com/cdn-cgi/trace`, 15s, 8. No filter type of its own and no gate: a bad endpoint costs tag accuracy, never survivors. There is no per-entry override — `filters` cannot name it, so `geoblock.geotrace` is the only place it is configured
...
- `(*Config).AnnotateUsesProvider(name string) bool` — true when any `annotate` chain names the provider. The stable worker asks for `ProviderGeoTrace` to decide whether to run the trace probe at all: it costs one request through every survivor, so an offline-only chain must never arm it
...
- `Egress{IP, Country}` / `Valid()` — what a node reported about ITSELF through the `geotrace` probe; the zero value means it was never measured (always, on `/`)
...
| `stable`, `probe`, `url-test`, `gemini`, `claude`, `chatgpt`, `tidal`, `geotrace`, `bandwidth`, `worker`, `mihomo` | `stable` |
