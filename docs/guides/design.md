...
- Parsing is **generic URI parsing**, not hardcoded to `vless://` only, and that stays the policy: there is deliberately **no whitelist** of mihomo-known schemes, so a scheme with no concrete reason to be rejected keeps the generic `scheme://[userinfo@]host[:port][?query][#fragment]` walk. Four schemes need more than that walk, because the fields the pipeline needs are not where it looks, and each decoder mirrors mihomo's so that a node we keep is a node the prober can convert:
  - `vmess://` — two share-link bodies, both accepted on mihomo's own terms: the V2RayN base64 JSON (`add`/`port`/`ps`) and the Xray VMessAEAD authority `uuid@host:port[?query][#name]`. `parseVmess` dispatches on an `@` in the pre-fragment body (`vmess.go:35-36`) — the same discriminator as mihomo's base64 attempt, since `@` is in no base64 alphabet (`convert/converter.go:236-239`) — and the AEAD arm mirrors `handleVShareLink`'s gates: host AND a digit-only port, both required (`convert/v.go:17-22`; any other port text fails mihomo's `url.Parse`), while the user part is deliberately NOT validated — mihomo maps a uuid string `uuid.FromString` rejects through `UUIDMap` to a deterministic UUIDv5 (`transport/vmess/vmess.go:87`, `common/utils/uuid.go:46-51`), so a non-UUID user still dials and is kept. A JSON body whose `port` reads empty is refused rather than fabricated to 443, and a url-safe base64 body is re-encoded to the standard alphabet at parse, because mihomo's vmess decode accepts the STD alphabets only (`convert/base64.go:24-33`). The AEAD name is relabeled through `subscription.RewriteVmessAEADName` — the fragment twin of `RewriteVmessName`, reached by the payload rewriter's false return in `rewrite.NodeName` and `merge.relabelNode`.
  - `ss://` — the PORT picks the form, as it does for mihomo: a portful SIP002 link takes the untouched generic path, and a portless authority is the legacy form, base64 of `method:pass@host:port`, decoded with `base64.RawStdEncoding` ONLY. Narrower than our tolerant vmess decoder on purpose: the shadowsocks spec writes that form WITHOUT padding and mihomo decodes it with that alphabet alone, so a padded or url-safe payload is refused at parse (the authority never decodes) where a tolerant decoder — the vmess habit — would have kept it, and a line we kept but mihomo cannot decode is a node that merges and converts to nothing — an attributed `unconvertible` refusal (see the refusal-account decision below) re-judged every cycle, never published and never dead-cached, where an honest parse-time reject costs one `Unsupported` booking. Branching on the `@` instead would keep `ss://<b64userinfo>@host` — SIP002-shaped but portless, and a line mihomo drops — and the blanket 443 default the parser once applied would have published it under a port it does not have; the parser now refuses the line outright too, because a portless ss authority must decode as legacy base64 (`decodeSSLegacy`), an alphabet holding no `@`.
...
- A **portless line is refused for every scheme except `hysteria2`/`hy2`**, which receive the 443 mihomo itself supplies (`converter.go:85-89`): mihomo drops a portless vless fail-closed on `url.Port() == ""` (convert/v.go:20-22), the empty trojan/tuic/hysteria port fails its structure decoder, and an unknown scheme has no converter case — and vmess is not on this generic branch, its two bodies refusing the portless shapes in their own decoders (see the `vmess://` bullet) — so defaulting the port (as the parser once did for every scheme) published a probe slot and a dead-cache entry under a port the node does not have. The PORTFUL form is still a valid node under the generic walk — which is why `internal/classify`'s proxy-scheme whitelist and the crawler's inline-node scheme list (`isInlineScheme`) still leave the plain-web schemes out: a `https://example.com:8443/docs` in a README must not make the page read as a live subscription, and the portless half of that (`https://t.me/somechannel`, an `http://` panel notice) dies at parse for every scheme, not only those five.
- **The Xray VMessAEAD body is a first-class vmess node — accepted under mihomo's own gates, not refused wholesale and not defaulted into shape.** The old parser refused every non-base64 vmess body, and the portless-rule narrative above treated the whole form as fail-closed; today a portful `uuid@host:port` authority parses, under the gate copied from `handleVShareLink` (host AND digit-only port required, `convert/v.go:17-22`) rather than a generous accept. Generosity would mean keeping lines mihomo's converter drops — fabricating a missing port, or accepting a hostless authority — and a kept-but-unconvertible node merges, converts to nothing, and occupies the pool as an attributed refusal that can never be published, where an honest parse-time reject costs one `Unsupported` booking. The one place the generous reading was REFUTED is the user part: a non-UUID "uuid" is not refused by mihomo — the mapping's uuid string feeds `UUIDMap`, which turns a string `uuid.FromString` rejects into a deterministic UUIDv5 of the string (`transport/vmess/vmess.go:87`, `common/utils/uuid.go:46-51`) — so it is accepted here too, and pinned by test. The port-443 fabrication for a portless JSON body is gone for the same reason: mihomo's structure decode refuses the empty port outright (`common/structure/structure.go:143-148`), and a node mihomo cannot convert must not sit in the probe queue under a port it does not have.
...
- **No downloadable COUNTRY source can refuse the boot on a feed outage, unless the refresh is explicitly disabled; the allow-list can, unconditionally.** A geofeed load that parses nothing warns, starts empty and retries (`initialGeofeedState`), exactly as dbip and registry do — the asymmetry that made geofeed fatal bought nothing: a feed outage took the whole service down, where an empty geofeed costs one retry delay of its MARGINAL contribution — the `GEO` tag reads the whole chain and the country filter reads its LOCAL half (`localCountryProvider` drops `cloudflare` and `asn`), so with the shipped `providers: [cloudflare, geofeed, dbip, registry]` dbip and registry keep placing nodes (measured: empty geofeed + populated dbip still answers an allow-list `kept=1, geo_drop=0`), and the answer empties only where the chain names no other LOCAL provider — including a chain that names only `cloudflare`, whose local half is empty and falls back to the geofeed alone. Three refusals are deliberate: an explicit `refresh_interval: 0` keeps a failed geofeed load fatal, because nothing would ever retry and the emptiness is silent; the same rule rejects an EMPTY lookup carried across a reload into a refresh-disabled config, which would strand that state with no warning at all; and the `cidr` allow-list below fails closed because an undownloaded allow-list drops every node rather than placing none.
...
- **A probed node mihomo could not turn into a URL test is a REFUSAL, not a death — accounted, never a stage.** `MihomoProber` carries a parse-refusal account (`CycleReport.Refusals`, a `RefusalReport` rendered on the trace family's absent gate): a node is `unconvertible` when `convert.ConvertsV2Ray` never emitted a mapping for its line — the share-link converter has no case for the scheme (wireguard/wg 32, ssh 10, amneziawg and vmvless 1 each in the measured corpus) or the shape failed the decode — and `unparsable` when the converter DID build a mapping and `adapter.ParseProxy` refused it: an unknown ss cipher, an invalid UUID. Neither class is a probe outcome and neither is a liveness verdict — no URL-test round was spent on it, and a parse-refused node that passed the TCP pre-check had its endpoint judged ALIVE. A `RefusalRan` account renders both reasons every cycle, zeros included, as `stable_probe_refused_nodes{reason="unconvertible"|"unparsable"}`, and `ProbeStages` sums with `Refusals` to `Probed`; a report without the account (the zero value) renders no refusal series at all, and `stage="unknown"` then means only that a prober without the account left a result-map miss unnamed.
- **A node mihomo could never map is NOT written into the dead cache — it says nothing about its endpoint.** `recordDead` blocks exactly what its probe judged dead — result-map entries with zero successful rounds (URL-test failure through the tunnel, or a pre-check-condemned endpoint) — and, when the refusal account ran, NEVER a result-map miss: the converter dropped the line or `adapter.ParseProxy` refused the mapping, no URL round is saved by caching it (a refused mapping never spends one), and a `server:port` entry would shadow a working sibling under `Merge`'s first-wins dedupe for the whole jittered TTL — the ssr-relabel regression's exact shape. The absence-means-death rule survives only for a prober WITHOUT the account, which nothing obliges to name every label. The write's plausibility breaker is unchanged, but for an accounted cycle its denominator is the entries the probe actually JUDGED (the result-present ones) — never the refused, which cannot be blocked and would hold a wholly refused pool under the trip threshold exactly as unresolvable endpoints would in `filterReachable`'s: the breaker must stay about our egress, and a refusal is not an egress verdict.
